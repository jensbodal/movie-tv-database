...
SELECT actor.id AS actor_id, first_name, last_name, birthdate FROM person 
  INNER JOIN actor ON actor.person_id = person.id 
  ORDER BY first_name;

SELECT director.id AS director_id, first_name, last_name, birthdate FROM person 
  INNER JOIN director ON director.person_id = person.id 
  ORDER BY first_name;

SELECT genre.genre_type AS genre FROM genre
        ORDER BY genre ASC;

SELECT person.id AS person_id, first_name, last_name, birthdate FROM person 
  ORDER BY first_name;

SELECT id, title, start_year, end_year, release_country, content_rating FROM tvshow 
  ORDER BY title;
